...
Spring JPA와 MyBatis를 이용한 개발 차이를 알아보자. Spring JPA에는 QueryDSL를 추가하여 가급적 실무에서 많이 사용하는 방법을 이용했다. 그외 Spring 설정은 간소화하였고, 게시판 개발등을 통해 비즈니스 로직 구현시 실제 차이점을 알아보도록 한다. 여기에는 두 구성에 대한 차이점만 확인하는 것이므로 jpa, mybatis의 설명은 하지 않겠다.
